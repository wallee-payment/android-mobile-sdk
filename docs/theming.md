...
```kotlin
// ...
import com.wallee.enums.ThemeEnum
...
Use `setAnimation` method to change the screen change animation. Currently available options are: `AnimationEnum.SLIDE` and `AnimationEnum.BUBBLE`. Default value is `AnimationEnum.SLIDE`. `WalleePaymentSdk.instance?.setAnimation(AnimationEnum.BUBBLE)` changes the screen transition animation used inside the payment flow.
